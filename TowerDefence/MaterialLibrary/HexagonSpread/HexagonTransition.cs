...
namespace MaterialLibrary.HexagonSpread
{
    ...
    public class HexagonTransition : MonoBehaviour
    {
        ...
        public void RegenerateMesh()
        {
            ...
            _hexagonMesh.vertices = new Vector2[] {
                top,  // 0
                tleft, tright,  // 1, 2
                bleft, bright,  // 3, 4
                bottom  // 5
        ...
        private void RegenerateGrid()
        {
            ...
            for (int y = -iy; y <= iy; y++)
                for (int x = -ix; x <= ix; x++)
                    vectors.Add(new Vector2(x * _gridHexSize * magicNumberHalf, y * _gridHexSize * 1.5f) + Position);

            // Second half of the grid (the offset ones)
            for (int y = -jy; y < jy; y++)
                for (int x = -jx; x < jx; x++)
                    vectors.Add(new Vector2((x + .5f) * _gridHexSize * magicNumberHalf, (y + .5f) * _gridHexSize * 1.5f) + Position);
        ...
        public override void _Process(double delta)
        {
            ...
            Graphics.DrawMeshInstancedProcedural(
                ...
                submeshIndex: 0,
                CurrentMaterial,
                new Bounds(Position, new Vector2(_viewWidth, _viewHeight)),
                count: _hexagonCount,
                camera: _camera,
                layer: _layer
                );
